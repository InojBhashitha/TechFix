...
public class AdminMainActivity extends AppCompatActivity implements StaffRepairQueueAdapter.OnStaffRepairActionListener {
    ...
    private ChipGroup chipGroupBranch;

    private StaffRepairQueueAdapter queueAdapter;
    private Long currentBranchFilter = null; // null = All Branches, 1 = Colombo, 2 = Galle
    ...
    private void initViews() {
        ...
        btnBranchFilter = findViewById(R.id.btnBranchFilter);

        swipeRefreshLayout = findViewById(R.id.swipeRefreshLayout);
        rvRepairQueue = findViewById(R.id.rvRepairQueue);
        layoutEmpty = findViewById(R.id.layoutEmpty);
        chipGroupBranch = findViewById(R.id.chipGroupBranch);
    ...
    private void setupHeader() {
        tvWelcome.setText("System Administrator");
        tvRole.setText("Global Operations Oversight • " + sessionManager.getEmail());
    }
    ...
    private void setupListeners() {
        ...
        chipGroupBranch.setOnCheckedStateChangeListener((group, checkedIds) -> {
            if (checkedIds.isEmpty()) {
                currentBranchFilter = null;
            } else {
                int checkedId = checkedIds.get(0);
                if (checkedId == R.id.chipColombo) {
                    currentBranchFilter = 1L; // Colombo
                } else if (checkedId == R.id.chipGalle) {
                    currentBranchFilter = 2L; // Galle
                } else {
                    currentBranchFilter = null; // All Branches
                }
            }
            refreshAllData();
        });
    ...
    private void loadDashboardStats() {
        apiService.getStaffDashboardStats(currentBranchFilter).enqueue(new Callback<ApiResponse<StaffDashboardStatsDto>>() {
            @Override
    ...
    private void loadRepairQueue() {
        ...
        apiService.getStaffBookings(currentBranchFilter, null).enqueue(new Callback<ApiResponse<List<BookingResponseDto>>>() {
            @Override
            public void onResponse(Call<ApiResponse<List<BookingResponseDto>>> call, Response<ApiResponse<List<BookingResponseDto>>> response) {
                ...
                if (response.isSuccessful() && response.body() != null && response.body().isSuccess()) {
                    ...
                    Toast.makeText(AdminMainActivity.this, "Failed to fetch system repair queue", Toast.LENGTH_SHORT).show();
                }
            }
            ...
            public void onFailure(Call<ApiResponse<List<BookingResponseDto>>> call, Throwable t) {
                swipeRefreshLayout.setRefreshing(false);
                Toast.makeText(AdminMainActivity.this, "Network error: " + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void showUpdateStatusDialog(BookingResponseDto booking) {
        ...
        tvDialogBookingRef.setText("Admin Override: " + booking.getBookingReference() + " • Current: " + currentStatusName);
        ...
        btnSubmitStatus.setOnClickListener(v -> {
            ...
            if (selectedIndex < 0 || selectedIndex >= STATUS_ENUM_KEYS.length) return;
            ...
            apiService.updateRepairStatus(booking.getBookingReference(), request).enqueue(new Callback<ApiResponse<BookingResponseDto>>() {
                ...
                public void onResponse(Call<ApiResponse<BookingResponseDto>> call, Response<ApiResponse<BookingResponseDto>> response) {
                    ...
                    if (response.isSuccessful() && response.body() != null && response.body().isSuccess()) {
                        Toast.makeText(AdminMainActivity.this, "Status updated by Admin successfully", Toast.LENGTH_SHORT).show();
                        dialog.dismiss();
                        refreshAllData();
                    } else {
                        Toast.makeText(AdminMainActivity.this, "Failed to update status", Toast.LENGTH_SHORT).show();
                    }
                }
    ...
    private void showAssignTechnicianDialog(BookingResponseDto booking) {
        ...
        tvDialogAssignRef.setText("Admin Assign: " + booking.getBookingReference() + " • " + (booking.getServiceName() != null ? booking.getServiceName() : ""));
            ...
            public void onResponse(Call<ApiResponse<List<TechnicianDto>>> call, Response<ApiResponse<List<TechnicianDto>>> response) {
                if (response.isSuccessful() && response.body() != null && response.body().isSuccess()) {
                    ...
                    if (technicians == null || technicians.isEmpty()) {
                        Toast.makeText(AdminMainActivity.this, "No technicians found", Toast.LENGTH_SHORT).show();
                        dialog.dismiss();
                        return;
                    }
                    ...
                    ArrayAdapter<String> techAdapter = new ArrayAdapter<>(AdminMainActivity.this, android.R.layout.simple_spinner_item, spinnerLabels);
                        ...
                        apiService.assignTechnician(booking.getBookingReference(), assignRequest).enqueue(new Callback<ApiResponse<BookingResponseDto>>() {
                            ...
                            public void onResponse(Call<ApiResponse<BookingResponseDto>> call, Response<ApiResponse<BookingResponseDto>> assignResponse) {
                                ...
                                if (assignResponse.isSuccessful() && assignResponse.body() != null && assignResponse.body().isSuccess()) {
                                    Toast.makeText(AdminMainActivity.this, "Assigned technician " + selectedTech.getFullName() + " successfully", Toast.LENGTH_SHORT).show();
                                    dialog.dismiss();
                                    refreshAllData();
                                } else {
                                    Toast.makeText(AdminMainActivity.this, "Failed to assign technician", Toast.LENGTH_SHORT).show();
                                }
                            }
    ...
    private void loadInventoryData(Boolean lowStockOnly, StaffInventoryAdapter adapter, SwipeRefreshLayout refreshLayout, RecyclerView recyclerView, LinearLayout emptyLayout) {
        ...
        apiService.getInventory(currentBranchFilter, lowStockOnly).enqueue(new Callback<ApiResponse<List<InventoryStockDto>>>() {
            @Override
    ...
    private void showUpdateStockDialog(InventoryStockDto item, Runnable onUpdated) {
        ...
        btnSaveStock.setOnClickListener(v -> {
            ...
            if (qtyStr.isEmpty() || minAlertStr.isEmpty()) return;
            ...
            apiService.updateInventoryStock(item.getId(), updateRequest).enqueue(new Callback<ApiResponse<InventoryStockDto>>() {
                ...
                public void onResponse(Call<ApiResponse<InventoryStockDto>> call, Response<ApiResponse<InventoryStockDto>> response) {
                    ...
                    if (response.isSuccessful() && response.body() != null && response.body().isSuccess()) {
                        Toast.makeText(AdminMainActivity.this, "Stock updated by Admin successfully!", Toast.LENGTH_SHORT).show();
                        stockDialog.dismiss();
